...
// Reads the current index and its ETag. Returns an empty list (no etag) if
// the index doesn't exist yet — that's expected on the very first upload.
async function readIndexRaw() {
  let meta;
  try {
    meta = await head(INDEX_PATH);
  } catch {
    return { list: [], etag: null };
  }
